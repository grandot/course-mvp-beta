...
async function simulateTaskExecution(intent, slots, userId) {
  try {
    ...
    // 動態載入對應的任務處理器
    const taskHandlers = {
      'add_course': '../src/tasks/handle_add_course_task',
      'create_recurring_course': '../src/tasks/handle_add_course_task',
      'query_schedule': '../src/tasks/handle_query_schedule_task',
      'record_content': '../src/tasks/handle_record_content_task',
      'add_course_content': '../src/tasks/handle_record_content_task',
      'set_reminder': '../src/tasks/handle_set_reminder_task',
      'cancel_course': '../src/tasks/handle_cancel_course_task',
      'stop_recurring_course': '../src/tasks/handle_cancel_course_task'
    };
    
    if (!taskHandlers[intent]) {
      return {
        success: false,
        message: `❓ 任務處理器 ${intent} 尚未實作`
      };
    }
    
    const handler = require(taskHandlers[intent]);
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('📖 使用方法:');
    console.log('  單一測試: node tools/send-test-message.js "測試訊息"');
    console.log('  批量測試: node tools/send-test-message.js --batch');
    console.log('');
    console.log('📝 測試範例:');
    console.log('  node tools/send-test-message.js "小明每週三下午3點數學課"');
    console.log('  node tools/send-test-message.js "查詢小明今天的課程"');
    return;
  }
