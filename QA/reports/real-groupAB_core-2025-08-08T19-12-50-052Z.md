...
- 批次: groupAB_core
- 產出時間: 2025-08-08T19:12:50.052Z
- 測試數: 35 | 通過: 23 | 失敗: 12 | 通過率: 66%
...
- 診斷：
  ...
  - systemBehavior:
    ...
    - "message": "🚀 生產用戶，使用真實 LINE Service",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754679950097",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680026334",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - taskExecution:
    ...
    - "message": "📤 [測試模式] 實際業務回覆: ❓ 請提供以下資訊：課程日期",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680043883",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🚀 生產用戶，使用真實 LINE Service",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680088944",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "📤 [測試模式] 實際業務回覆: 處理訊息時發生錯誤，請稍後再試。",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🚀 生產用戶，使用真實 LINE Service",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680104204",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "📤 [測試模式] 實際業務回覆: 處理訊息時發生錯誤，請稍後再試。",
...
- 診斷：
  ...
  - slotExtraction:
    ...
    - "message": "📋 提取結果: { studentName: '我', content: '課程安排的情況' }",
  - taskExecution:
    - "message": "🎯 執行任務: query_schedule",
    - "message": "📋 接收參數: { studentName: '我', content: '課程安排的情況' }",
    - "message": "📊 任務執行結果: {",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680119497",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680148168",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  - intentParsing:
    - "message": "🎯 開始解析意圖: 測試小明明天下午2點要上測試數學課 (用戶: U_test_user_qa)",
  - slotExtraction:
    - "message": "🕒 開始高級時間解析: 已補充 courseDate: 測試小王今天有什麼課？",
    - "message": "❌ 時間解析失敗: 已補充 courseDate: 測試小王今天有什麼課？",
  - taskExecution:
    - "message": "📋 接收參數: {",
    - "message": "📊 任務執行結果: {",
    ...
    - "message": "      message: '✅ 課程已安排成功！\\n\\n👦 學生：測試小光\\n📚 課程：十分測試英文課\\n📅 日期：2025-08-09\\n🕐 時間：上午10:30',",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680207795",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🚀 生產用戶，使用真實 LINE Service",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680237357",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680254915",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
...
- 診斷：
  ...
  - taskExecution:
    - "message": "📊 任務執行結果: {",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680313541",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754680354742",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
