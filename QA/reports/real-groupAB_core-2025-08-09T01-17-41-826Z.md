...
- 批次: groupAB_core
- 產出時間: 2025-08-09T01:17:41.826Z
- 測試數: 35 | 通過: 26 | 失敗: 9 | 通過率: 74%
...
- 診斷：
  ...
  - slotExtraction:
    ...
    - "message": "🕒 開始高級時間解析: 測試小明每月1號早上8點英文課",
    - "message": "✅ 時間解析成功: {",
  ...
  - systemBehavior:
    ...
    - "message": "🔍 檢查 replyToken: test-reply-token-1754701914625",
...
- 診斷：
  - intentParsing:
    - "message": "🎯 開始解析意圖: 測試小明每天下午2點測試數學課 (用戶: U_test_user_qa)",
  - taskExecution:
    ...
    - "message": "  message: '❓ 請提供以下資訊：課程日期\\n\\n範例：「小明每週三下午3點數學課」',",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754701929004",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754701929004",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
...
- 診斷：
  ...
  - systemBehavior:
    ...
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754701946886",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - slotExtraction:
    - "message": "🔍 開始提取 slots - 意圖: add_course (用戶: U_test_user_qa)",
    - "message": "🕒 開始高級時間解析: 測試小明每天早上8點英文課",
    - "message": "✅ 時間解析成功: {",
    - "message": "📊 規則提取置信度: 0.50",
    - "message": "🤖 啟用 AI 輔助提取...",
    - "message": "✅ 最終 slots: {",
  - taskExecution:
    ...
    - "message": "📤 [測試模式] 實際業務回覆: ❓ 請提供以下資訊：課程日期",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754702008412",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754702052789",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754702143803",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  - taskExecution:
    - "message": "📊 任務執行結果: {",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754702161109",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - taskExecution:
    ...
    - "message": "📊 任務執行結果: {",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754702177569",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  - taskExecution:
    - "message": "📊 任務執行結果: {",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754702216608",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "📤 [測試模式] 實際業務回覆: ✅ 課程內容已成功記錄！",
