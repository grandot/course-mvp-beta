...
- 診斷：
  ...
  - systemBehavior:
    ...
    - "message": "🚀 生產用戶，使用真實 LINE Service",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681439460",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681518032",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🚀 生產用戶，使用真實 LINE Service",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681576669",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "📤 [測試模式] 實際業務回覆: 處理訊息時發生錯誤，請稍後再試。",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🚀 生產用戶，使用真實 LINE Service",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681592076",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "📤 [測試模式] 實際業務回覆: 處理訊息時發生錯誤，請稍後再試。",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681630779",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "📤 [測試模式] 實際業務回覆: ❌ 目前不支援「correction_intent」功能，請稍後再試",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681629982",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  - taskExecution:
    - "message": "📊 任務執行結果: {",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681713251",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681731945",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - taskExecution:
    ...
    - "message": "📊 任務執行結果: {",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681788784",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
...
- 診斷：
  ...
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681805910",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
...
- 診斷：
  ...
  - taskExecution:
    - "message": "  message: '❓ 請提供以下資訊：學生姓名、上課時間\\n\\n範例：「小明每週三下午3點數學課」',",
    - "message": "📤 [測試模式] 實際業務回覆: ❓ 請提供以下資訊：學生姓名、上課時間",
    - "message": "📋 接收參數: {",
    - "message": "📊 任務執行結果: {",
    - "message": "  message: '❓ 請提供以下資訊：學生姓名、上課時間\\n\\n範例：「小明每週三下午3點數學課」',",
    - "message": "  message: '❓ 請提供以下資訊：學生姓名、上課時間\\n\\n範例：「小明每週三下午3點數學課」',",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681826543",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681826543",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🚀 生產用戶，使用真實 LINE Service",
...
- 診斷：
  - taskExecution:
    ...
    - "message": "📤 [測試模式] 實際業務回覆: ❓ 請提供以下資訊：上課時間、課程日期",
  - systemBehavior:
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681858617",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
    - "message": "🔍 檢查 replyToken: test-reply-token-1754681859167",
    - "message": "🧪 檢測到測試 token，跳過真實 LINE API 調用",
