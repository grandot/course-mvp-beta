...
### 測試範圍
- **Group A: 獨立功能**（35個用例）- 無需前置資料
- **Group B: 依賴功能**（40個用例）- 基於Group A建立的資料
- **Group C: 複雜操作**（20個用例）- 需完整數據環境
...
- **測試方法**：
  1. ENABLE_DAILY_RECURRING=false 時測試「測試小明每天早上8點英文課」
  2. ENABLE_DAILY_RECURRING=true 時測試相同輸入
- **預期結果**：
  1. 功能關閉時：優雅降級到非重複模式
  2. 功能開啟時：正確處理每日重複
- **實際回覆**：（待填）
...
- **測試序列**：
  1. 建立：「測試小明每天下午2點測試數學課」
  2. 衝突：「測試小明每天下午2點測試英文課」
...
- **測試序列**：
  1. 取消每日課程：「取消測試小明每天的測試晨練課」
  2. 取消每週課程：「取消測試Lumi每週三的測試鋼琴課」
...
### 執行時程
- **Group A**：預計1.5小時（35個用例）
- **Group B**：預計2小時（40個用例，包含數據驗證）
- **Group C**：預計1小時（20個用例）
- **總計**：4.5小時
