...
## 2025-08-16 - 修復本週課表日期範圍錯誤（UTC時區轉換問題）

### 🐛 Fixed
- **課表日期範圍修復**：解決查詢本週課表顯示錯誤日期範圍問題（8/9~8/16 → 8/10~8/16）
- **時區轉換問題**：修正 UTC 時區轉換導致日期減一天的 BUG
- **重複課程展開**：修復 daily 和 weekly 重複課程的日期格式化邏輯

### 🔧 Technical
- 修改 `src/tasks/handle_query_schedule_task.js` 第 216、244 行
- 替換 `toISOString().split('T')[0]` 為本地日期格式化
- 使用 `getFullYear()`, `getMonth()`, `getDate()` 避免 UTC 轉換
- 台北時區 `2025-08-10T00:00:00+08:00` 不再轉為 `2025-08-09`

### 📊 問題根因
- **問題現象**：當前日期 2025-08-16（週六），本週應為 8/10~8/16，但顯示 8/9~8/16
- **根本原因**：`toISOString()` 將台北時區午夜轉為 UTC，時間減 8 小時變成前一天
- **影響範圍**：所有重複課程的日期展開邏輯（expandRecurringCourses 函數）

### 🧪 協作驗證
- **第一性原則分析**：Codex CLI (GPT-5) 精準定位時區轉換為根因
- **修復方案執行**：Claude Code 實施本地日期格式化修復
- **驗證結果**：週間範圍正確顯示為 2025-08-10 ~ 2025-08-16 ✅
...
### 🐛 Fixed
- **時間解析修復**：修正後備時間解析邏輯中「半點」判斷過於寬泛的問題
- **課程名稱污染**：解決「三點半」的「半」字錯誤污染課程名稱的 BUG
- **精確匹配**：改用匹配片段級別的「半」字判斷，避免全句掃描

### 🔧 Technical
- 修改 `src/intent/extractSlots.js` 第 108 行
- 將 `message.includes('半')` 改為 `match[0].includes('半')`
- 確保只有當前時間匹配片段包含「半」時才設為 30 分

### 📊 測試結果
- ✅ 輸入：「Lumi每週一三五下午三點半趣味科學課」
- ✅ 正確提取：scheduleTime=15:30, courseName=趣味科學課
- ✅ 不再出現：courseName=半趣味科學課（污染）

---

## 2025-08-15 - 修復週間多日重複課程查詢展開問題

### 🐛 Fixed
- **查詢課表功能修復**：修正 `expandRecurringCourses` 函數無法展開週間多日重複課程的問題
- **陣列支援**：支援 `dayOfWeek` 為陣列格式（如 `[1,3,5]` 代表週一三五）的重複課程展開
- **查詢結果完整性**：確保查詢「下週課表」時，週一三五的重複課程能完整顯示所有三天

### 🔧 Technical
- 修改 `src/tasks/handle_query_schedule_task.js` 第 224-248 行
- 將 `dayOfWeek` 正規化為 `Set` 結構，支援單一數字或陣列格式
- 使用 `daysSet.has(d.getDay())` 判斷日期是否匹配重複規則

### 📊 測試結果
- ✅ 單一天重複課程（如每週三）正常顯示
- ✅ 多天重複課程（如每週一三五）完整展開為三個獨立日期
- ✅ 向下相容：原有單一數字格式 `dayOfWeek` 仍正常運作

---

## 2025-08-15 - 修復取消課程課名匹配與 QuickReply 上下文問題
...
- **QuickReply 上下文修復**：修復「刪除整個重複」按鈕點擊後仍要求課程名稱的問題
- **課程名稱提取修復**：修復「刪除有趣味科學課」被錯誤提取為「除有趣味科學課」的問題
- **重複課程刪除修復**：確保「刪除整個重複」正確調用 deleteRecurringEvent 刪除整個系列
- **時間片段污染修復**：修復「Lumi每週一三五下午三點半有趣味科學課」被錯誤提取為「半趣味科學課」的問題
...
- **QuickReply 上下文保存**：`handle_cancel_course_task.js` 247-271行添加 `setExpectedInput` 調用
- **自動補全條件放寬**：`extractSlots.js` 1207行允許缺少課程名稱時也能使用上下文推導
- **動作詞清洗增強**：在 `extractSlots.js` 五處位置加入「刪除/删除」清洗邏輯，避免正則錯誤匹配
- **重複課程刪除邏輯完善**：確保 scope 正確傳遞至 course 物件，優化 calendarId 查找，改進範圍回應訊息
- **時間片段預清洗**：`extractSlots.js` 468-474行新增時間表達式清洗，避免「點半」「HH:mm」「上午/下午」殘留影響課名匹配
...
- ✅ 保持其他取消功能正常運作
- ✅ QuickReply 按鈕上下文保存機制已實現
- ✅ 「刪除有趣味科學課」正確提取課程名稱為「有趣味科學課」
- ✅ 「刪除整個重複」正確調用 deleteRecurringEvent 並顯示對應訊息
- ✅ 「Lumi每週一三五下午三點半有趣味科學課」正確提取課程名稱為「有趣味科學課」
