...
function extractStudentName(message) {
  ...
  const namePatterns = [
    ...
    // 高精確度模式 - 專門處理新增類型的語句
    /(?:新增|幫.*?新增|安排)\s*([小大]?[一-龥A-Za-z]{2,6})(?=星期|週|[今昨明]天|下午|上午|晚上|中午)/, // 新增小明星期、新增Lumi下午
    ...
    /^([小大]?[一-龥A-Za-z]{1,6})每個?月/, // 小明每月...、Lumi每個月...
...
function extractCourseName(message) {
  ...
  const coursePatterns = [
    // 最高精確度模式 - 明確的課程結構  
    /每個?月\d{1,2}號要([一-龥A-Za-z]{2,8})/, // 每月20號要數學測驗 (更具體，優先)
    /每個?月\d{1,2}號([一-龥A-Za-z]{2,8}課?)/, // 每月20號數學測驗課
...
function stripTimeSuffixFromName(name) {
  ...
  const stripped = name
    ...
    .replace(/(本月|每月|下月|這月|上月)$/, ''); // 避免月份詞被當成學生名
...
function cleanCourseName(rawCourse) {
  ...
  const cleaned = rawCourse
    .replace(/^(設定|不要|取消|刪掉|刪除|删除|查詢|記錄|要)/, '');
