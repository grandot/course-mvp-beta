...
## [Hotfix 6.3.1 - 修改課程功能Bug修復] - 2025-07-25

### 🐛 Bug修復
- **課程名稱識別問題**: 修復"直排輪"無法匹配"直排輪課"的問題
- **意圖判斷錯誤**: 修復"直排輪課改成明天上午10點"被誤判為新增課程的問題

### 🔧 技術修復
- **intent-rules.yaml**: 提升modify_course優先級到12，新增"改成"、"改到"等關鍵字
- **SemanticService**: 新增課程名稱模糊匹配功能，支持雙向包含匹配
- **模糊匹配邏輯**: "直排輪" ↔ "直排輪課"、"數學" ↔ "數學課"等自動匹配

### 🎯 修復效果
```javascript
// 修復前
用戶: "直排輪改成明天上午10點"
系統: intent = record_course ❌ (錯誤判斷為新增)

// 修復後  
用戶: "直排輪課改成明天上午10點"
系統: intent = modify_course ✅ (正確識別為修改)
用戶: "直排輪改成明天上午10點"  
系統: course_name = "直排輪課" ✅ (模糊匹配成功)
```

### 🛠️ 模糊匹配規則
- **雙向包含**: 用戶輸入包含課程名稱 OR 課程名稱包含用戶輸入
- **大小寫不敏感**: 統一轉換為小寫進行比較
- **優雅降級**: 匹配失敗不影響原有功能
